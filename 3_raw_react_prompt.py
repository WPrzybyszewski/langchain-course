...
def extract_output_text(response) -> str:
    """Use assistant content first; fall back to thinking text for thinking models."""
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        print(f"LLM Output:\n{output}")
        if not output:
            print("  [Parsing] ERROR: Model returned empty content/thinking text")
            break
        ...
        action_match = re.search(r"^\s*Action:\s*(.+)$", output, flags=re.MULTILINE)
        action_input_match = re.search(
            r"^\s*Action Input:\s*(.+)$", output, flags=re.MULTILINE
        )
